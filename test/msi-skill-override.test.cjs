...
test("setup defaults mju-cli to the integration branch", () => {
  const setup = fs.readFileSync(path.join(root, "setup.sh"), "utf8");
  const dockerfile = fs.readFileSync(path.join(root, "Dockerfile"), "utf8");

  assert.match(setup, /MJU_CLI_BRANCH="\$\{MJU_CLI_BRANCH:-dev\}"/);
  ...
  assert.match(dockerfile, /git clone --branch dev https:\/\/github\.com\/university-claw\/mju-cli\.git/);
});
