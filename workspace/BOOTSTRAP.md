...
> **참고 (정상 흐름)**: `onboarding-gate` plugin이 inbound dispatch 단계에서 미온보딩 사용자 메시지를 1차로 가로챈다 — LLM 호출 자체가 일어나지 않고 plugin이 직접 modal을 발사한다. 따라서 정상 흐름에서는 미온보딩 사용자의 메시지가 너에게 전달되지 않는다. 만약 그런 메시지가 너에게 도달했다면 plugin이 실패했거나 비활성화된 fallback 상황이므로 아래 절차로 처리.
>
> 코드 레벨 게이트가 truth source. LLM은 fallback 안전망 역할.
